...
import staticData from './compiled_data.json';
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {paginatedData.map((row, index) => (
                                <div key={index} className={`p-4 border rounded-lg shadow-lg ${isDarkMode ? 'bg-gray-800 text-gray-100 border-gray-700' : 'bg-white text-gray-900 border-gray-300'} transition-transform transform hover:scale-105 flex flex-col min-w-0 flex-grow`}>
                                    ...
                                    <p className="text-sm"><strong>Сессия:</strong> {row.session}</p>
                                    <a href={row.responsible.startsWith('http') ? row.responsible : `https://${row.responsible}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">Ссылка на урок</a>
